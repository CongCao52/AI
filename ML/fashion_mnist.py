...
def load_data(file_path, reshape_images):
    ...
    labels = data[:,0].astype(np.longlong)
    raw_pixels = data[:,1:].astype(np.longlong)
    raw_height = np.shape(raw_pixels)[0]
    if reshape_images:
        pixels = raw_pixels.reshape((raw_height,1,28,28))
    else:
        pixels = raw_pixels
...
class EasyModel(torch.nn.Module):
    def __init__(self):
        ...
        self.function = torch.nn.Linear(784,10,bias = True)
    def forward(self, x):
        r = self.function(x)
        return r
...
class MediumModel(torch.nn.Module):
    def __init__(self):
        ...
        self.function1 = torch.nn.Linear(28*28, 14*14)
        self.function2 = torch.nn.Linear(14*14, 7*7)
        self.function3 = torch.nn.Linear(7*7, 10)
    def forward(self, x):
...
class AdvancedModel(torch.nn.Module):
    def __init__(self):
        ...
        self.cnn1 = torch.nn.Conv2d(in_channels=1, out_channels=6, kernel_size=5, stride=1, padding=2)
        self.relu = torch.nn.ReLU()
        self.cnn2 = torch.nn.Conv2d(in_channels=6, out_channels=20, kernel_size=5, stride=1, padding=2)
        self.function1 = torch.nn.Linear(20*7*7, 20*7) 
        self.function2 = torch.nn.Linear(20*7 , 35) 
        self.function3 = torch.nn.Linear(35, 10)
  

    def forward(self, x):
        x = F.max_pool2d(F.relu(self.cnn1(x)),2)
        x = F.max_pool2d(F.relu(self.cnn2(x)),2)
        x = x.view(x.size(0), -1)
        x = F.relu(self.function1(x))
        x = F.relu(self.function2(x))
        x = self.function3(x)
